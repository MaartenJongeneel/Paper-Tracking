function [pin,pface,pout,ed1,see_face] = compute_points(box,state,K,AH_M);
...
%            AH_M      : Relative transformation between A and M
%
...
MH_B = [state{1,1} state{2,1}; 0 0 0 1];    
AH_B = AH_M*MH_B;

state_A = {AH_B(1:3,1:3); AH_B(1:3,4)};
...
R_face_centers      = hom_trans(box.face_centers,state_A);
R_face_outer_points = hom_trans(outer_points,state_A);
...
pin{see_face(ii)}=persProj(hom_trans(box.Epoints{see_face(ii)},state),K,AH_M);
pface{see_face(ii)}=persProj(hom_trans(box.Spoints{see_face(ii)},state),K,AH_M);
end
...
ed1 = ed; %Output for plotting purposes
