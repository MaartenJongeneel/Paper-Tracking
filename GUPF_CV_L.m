...
%% Cuboid Tracking using Particle Filter with Constant Velocity model and measurement L
% Settings
...
for t=2:maxt%length(FileList)
    ...
    for i=1:Npart %For each particle
        ...
        [L(i),~,~,~]=likelihood(xPred(:,i),Href,hsi,K,box,AH_M);
    end
...
if DoSave
    GUPF_CV_L_Y = Y;
    save(sprintf(append('Results/','/GUPF_CV_L_P%dF%d'),Npart,maxt),'GUPF_CV_L_Y','runtime','GT','Z','K','AH_M','box','impacts');
end
...
figure('pos',[200 400 250 200]); 
    ...
    legend('GUPF\_CV\_L','L','location','southwest');
